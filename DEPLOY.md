...
- **2026-09-23 (~05:50 UTC)**: DB-11d change e-mail (`scripts/deploy-api.sh`, commit `f9d21de` + fixes `c0fbad8`), no migration (71 migrations remain). Endpoints `POST /api/me/change-email` and anonymous `POST /api/auth/confirm-email-change`; D14 conflict-never-merge.
- **2026-09-23 (~06:15 UTC)**: DB-13 operator impersonation (`scripts/deploy-api.sh`, commit `441a462` + `5f1fbc8` + fixes `790b9b8`), 72 migrations now in production (newest `20260923045038_AddImpersonationSessions`). Founder decision F2 live: metadata-only super admin by default; time-boxed (≤ 60 min) audited read-only impersonation session.
- **2026-09-23 (~10:30 UTC)**: DB-14 e-mail verification + password policy (`POINTER_APPLY_CONTRACT=1 POINTER_CONTRACT_LABEL=pre-db14 bash scripts/deploy-api.sh`, commit `335a993` + `7a042c3` + fixes `f0428a1`), 74 migrations now in production (newest `20260923062150_BackfillUsersEmailVerifiedAt`). R11 rehearsal on fresh dump: 19 s, 6/6 grandfathered live identities verified; production verified 6/6. Consequences live: admin-added members must verify e-mail before non-GET calls under `/api/admin/*` (`403` + `X-Email-Verification-Required`); passwords 10–128 not in embedded top-1000 list and not address. Deployed with settings fix `7bad6b5`.
- **2026-09-23 (~10:50 UTC)**: DB-15 activation funnel + usage rollup (`scripts/deploy-api.sh`, commit `2eabe72` + fixes `b2da554` + `a0363c6`), 75 migrations now in production (newest `20260923073100_AddUsageDailyAndWidgetInstalledIndex`). `usage_daily` live; rollup runs before 180 d usage sweep with high-water mark; endpoints `GET /api/admin/stats/funnel` (operator, metadata) and `GET /api/admin/stats/activation` (workspace).
- **2026-09-23 (~18:50 UTC)**: DB-16 screenshot purge (`scripts/deploy-api.sh`, commit `01a6eb9` + fixes `e1144a4` + `a3b5826`), 77 migrations now in production (newest `20260923153132_AddCommentsScreenshotPurgedAt`). First release runs with `Retention:ScreenshotPurgeDryRun=true` (logs only); flip to false after reviewing the purge log lines and a nightly tarball.
- **2026-09-23 (19:18 UTC)**: DB-17 demo as a product (`POINTER_APPLY_CONTRACT=1 POINTER_CONTRACT_LABEL=pre-db17 bash scripts/deploy-api.sh`, commit `da8cb3a` + fixes `f5288f2`), 79 migrations now in production (newest `20260923155947_BackfillWorkspacesDemoState`). Gate PASS + R11 rehearsal on the same-day dump first (doc §13); Q1–Q4 pre-checks 0 rows. Client `@moamen-ui/pointer-react@1.0.48`.
- **2026-09-23 (~20:41 UTC)**: CORS allow-list cleanup (`scripts/deploy-api.sh`, commit `9de4ba6`), no migration — retired `app-react.pointer.moamen.work` removed.
- **2026-09-23 (21:24 UTC)**: DB-11e drop legacy demo state (`POINTER_APPLY_CONTRACT=1 POINTER_CONTRACT_LABEL=pre-db11e bash scripts/deploy-api.sh`, commits `b8d75c9` + `2035749` + `7056c68`), 80 migrations now in production (newest `20260923205702_DropUsersLegacyDemoColumns`). Dropped `users.expires_at` + `IX_users_expires_at`, `"DemoExtended"`, `"DemoCommentCapOverride"`, `"DemoTtlHoursOverride"` (all empty per P3) and the `{id:int}` tenant routes (now 404). Client `@moamen-ui/pointer-react@1.0.49`.
- **2026-09-23 (~23:22 UTC)**: branded transactional e-mail layout (`scripts/deploy-api.sh`, commit `eedf977`), no migration — `EmailLayout` / `EmailComponents` / `EmailTemplateBuilder`.
- **2026-09-24 (00:33 UTC)**: DB-18 workspace self-service pause + e-mail/password-confirmed delete with a 7-day grace (`scripts/deploy-api.sh`, ordinary — additive; merge `36af150` + record `6303f53`), 81 migrations now in production (newest `20260923220301_AddWorkspacesPauseAndDeletionState`). Also fixes the pre-existing `HardDeleteAsync` FK violation on soft-deleted/erased identities. Gate PASS incl. the Mailpit lifecycle spec; R11 rehearsal on the 23:22 dump. Client `@moamen-ui/pointer-react@1.0.50`; dashboard (danger zone, `/confirm-workspace-deletion`, paused banner) deployed `c463777`.
- **2026-09-24 (~02:10 UTC)**: absolute screenshot URLs + DB-18 e-mails on the branded layout (`scripts/deploy-api.sh`, merge `8f97871`), no migration. Screenshot links are now `https://api.pointer.moamen.work/api/uploads/file?…` (built from `Pointer:PublicUrl` or the forwarded request origin); relative links had rendered the dashboard's `index.html` on the widget host / dashboard, so `<img>` showed its alt text. Arabic layout renders RTL on the cells, subjects use the raw workspace name.
- **2026-09-24 (~02:03 UTC)**: DB-16 screenshot purge switched from dry-run to live (`RETENTION_SCREENSHOT_PURGE_DRY_RUN=false` in `.env.prod`, `scripts/deploy-api.sh`). Reviewed first: dry-run lines showed 0 deleted-comment screenshots due and 0 orphans (4 files in 3 workspace folders, branding folder skipped); fresh local + off-site DB and uploads backups. First live sweep: `purged — 0 comment(s), 0 file(s), 0 failure(s)` and `swept — … 0 orphan file(s) … 0 failure(s)`.
- **2026-09-24 (~03:25 UTC)**: Arabic e-mail footer + remaining DB-18 tests (`scripts/deploy-api.sh`, merge `3df3ff0`), no migration.
- **2026-09-24 05:05 UTC**: DB-11f Part A — reads of `users.owner_id` / `approval_status` switched to memberships; `role_id` only for super admins; F1 role-write guard (`scripts/deploy-api.sh`, ordinary, merge `b282d02` + record `fdb5929`, deployed via agy), no migration (81). Pre-checks P1–P11 on the 03:00 dump all clean; main-vs-Part-A rehearsal identical; gate PASS ×2. First 3 min: 0 `DB-11f invariant` / 23503 / 42703 / Error lines. **Part B not before 2026-09-25 05:05 UTC**, after re-running P1 + P11 on production.
- **2026-09-24 (17:15 UTC)**: DB-11f Part B — contract deploy (`POINTER_APPLY_CONTRACT=1 POINTER_CONTRACT_LABEL=pre-db11f`, merge of `feat/db-11f-part-b` @ `be0022e`, run via agy with the gated `deploy-db11f-part-b.sh`), 83 migrations now in production (newest `20260924051230_ClearUsersRoleIdForMembers`). Dropped `users.owner_id`, `users.approval_status`, `ux_users_email_owner_live`, `IX_users_owner_id`, `fk_users_workspaces_owner_id`; `users.role_id` nullable and NULL for every non-super-admin. Soak shortened by the owner (no users today; ~12 h of clean Part A evidence). Pre-checks P1/P11/P6b 0, P9 3; post-deploy 0 error lines, health 200.
- **2026-09-24 (evening)**: login audit gap fixed (`scripts/deploy-api.sh`, commit `796fe73`, via agy), no migration — the choose-workspace and MFA-required login steps now write `auth.login.credentials_verified` (they wrote no row, logging `AUDIT GAP` on every multi-workspace or MFA login). Dashboard Security-log label deployed (`a39639f`). 0 error lines after deploy.
