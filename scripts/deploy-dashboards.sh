#!/usr/bin/env bash
# Rebuild and publish the React dashboard on the production VM.
#
# React is the only dashboard since 2026-09-15 — Angular and Vue were retired at tag
# `last-three-apps` / branch `legacy/angular-vue` in pointer-dashboard.
#
...
# Caddy serves ~/pointer-api/dashboard/react at app.pointer / demo.pointer (see Caddyfile).
...
rm -rf ~/pointer-api/dashboard/angular ~/pointer-api/dashboard/vue
