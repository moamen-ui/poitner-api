...
export default function pointerSource(options: PointerSourceOptions = {}) {
  ...
  async function writeManifest(): Promise<void> {
    ...
    //
    // No `generatedAt`. The file is regenerated by every build and is NOT committed (the
    // .gitignore `init` writes re-includes config.json, stack.json and pointer.sh, but never this
    // one — R3-01-01 asserts that). A timestamp would still be wrong: it would make two clean
    // clones produce different bytes, which is exactly what R3-01-05 forbids.
