...
**Class: Additive.** Ships as an ordinary `bash scripts/deploy-api.sh`. **Status 2026-09-22: written; not implemented.** Owner decisions D15.1–D15.4 have
defaults (§3.7); none blocks.
...
and "all workspaces" columns. **Weekly number** (D15.3): *activated workspaces per ISO week* = number of workspaces whose **earliest** `first_apply` row
...
`RetentionOptions` gains `public int RollupDays { get; init; } = 3;` (bound from `Retention:RollupDays`; `docker-compose.prod.yml:39-43` gets `Retention__RollupDays: "${RETENTION_ROLLUP_DAYS:-3}"`, `appsettings.json` `"RollupDays": 3`).
...
6. `API/Hosted/UsageRollup.cs` (§3.4); `RetentionService.cs` — `RollupDays` option + `SweepOnceAsync` ordering (§3.4); `API/appsettings.json:23-29` + `docker-compose.prod.yml:39-43` + `.env.prod.example` → `RollupDays`.
