...
|---|---|---|---|
| `auth.login.succeeded` | `LoginAsync` after `Issue(...)`; `LoginWithApiKeyAsync` (`after: {source: "api_key"}`); `LoginWithInviteAsync` (`source: "magic_link"`); `VerifyMfaLoginAsync` (`source: "mfa"`, actor override → SuperAdmin) | `user` / `public_id`; `owner` = the membership's workspace (super admin: NULL) | `after: {source}` |
| `auth.login.failed` | every non-success return of `LoginAsync` **and** `LoginWithApiKeyAsync`, `LoginWithInviteAsync` | identity found → `user`/`public_id` (override); else `email_hash`/hash (key login: `api_key`/id if resolvable, else `email_hash` = hash of the literal `"api_key"`) | `after: {reason: invalid_credentials \| passwordless \| pending \| rejected \| disabled \| locked \| no_workspace \| revoked_key}` |
| `auth.login.credentials_verified` | *(added post-launch — production AUDIT GAP fix)* `LoginAsync`'s two intermediate 200 branches: the DB-11b workspace picker (several live memberships — a correct password landing there is a SUCCESS envelope, not a failure) and R5-61's `mfa_required` challenge (super admin with TOTP enrolled). Both return a token but no session exists yet, so this row is written instead of a premature `auth.login.succeeded`; the real one follows later, from `SwitchWorkspaceAsync` (its own `auth.workspace_switched`) or `VerifyMfaLoginAsync` | `user`/`public_id` (override — the caller is anonymous); `owner` = NULL | `after: {source: "password", next: "choose-workspace" \| "mfa"}` |
