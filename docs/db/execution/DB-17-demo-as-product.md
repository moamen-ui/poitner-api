...
live member whose own home is elsewhere. **Orchestrator 2026-09-23 (review finding #3):** the first cut of that last guard rejected *any* second live
member, which meant a demo admin who had minted their own quick-access link or addressed invite (a real, demo-own member — its `owner_id` is this same
workspace) would silently make an otherwise-live, otherwise-eligible demo skip the backfill and never expire. Narrowed to reject only a second live
member whose identity's `owner_id` **IS DISTINCT FROM** `w.id` — i.e. a member genuinely at home somewhere else; a demo-own member no longer blocks
tagging. Hand-written `migrationBuilder.Sql(...)` in a migration with **nothing else** in it (scaffold with no model
...
WHERE u.is_demo = true
  ...
  AND NOT EXISTS (SELECT 1 FROM workspace_memberships o
                  ...
                  WHERE o.owner_id = w.id AND o.user_id <> u.id AND o.left_at IS NULL AND o.deleted_at IS NULL
                    AND ou.owner_id IS DISTINCT FROM w.id);  -- reject only a member whose home is elsewhere; a demo-own member (quick-access/invited by the demo admin) does not block tagging
```
`Down()`: `UPDATE workspaces SET demo_expires_at = NULL, demo_extended_at = NULL, demo_comment_cap_override = NULL, demo_ttl_hours_override = NULL, demo_expiry_warned_at = NULL WHERE demo_converted_at IS NULL;`
(reversible: the `users` columns still hold the source — that is what dual-write buys; `demo_expiry_warned_at` is nulled too — review finding #12 — even
though this backfill never writes it, so a rollback of only THIS migration cannot leave it disagreeing with the freshly-renulled `demo_expires_at`).
Marker line above `Up()` (copy `20260923062150:8-9`'s shape):
...
WHERE u.is_demo AND u.deleted_at IS NULL AND u.expires_at IS NOT NULL AND w.deleted_at IS NULL
  AND NOT EXISTS (SELECT 1 FROM workspace_memberships o
                  JOIN users ou ON ou.id = o.user_id
                  WHERE o.owner_id = w.id AND o.user_id <> u.id AND o.left_at IS NULL AND o.deleted_at IS NULL
                    AND ou.owner_id IS DISTINCT FROM w.id);
```
...
```
**Q4** (review finding #3) demos with any OTHER live member at all — split into demo-own (does not block tagging) vs. elsewhere-homed (does):
```sql
SELECT w.id AS workspace_id, o.user_id AS other_member_user_id, ou.owner_id AS other_member_home,
       (ou.owner_id IS DISTINCT FROM w.id) AS blocks_tagging
FROM workspaces w
JOIN users u ON u.owner_id = w.id AND u.is_demo AND u.deleted_at IS NULL AND u.expires_at IS NOT NULL
JOIN workspace_memberships m ON m.user_id = u.id AND m.owner_id = w.id AND m.left_at IS NULL AND m.deleted_at IS NULL
JOIN roles r ON r.id = m.role_id AND r.name = 'Workspace Admin'
JOIN workspace_memberships o ON o.owner_id = w.id AND o.user_id <> u.id AND o.left_at IS NULL AND o.deleted_at IS NULL
JOIN users ou ON ou.id = o.user_id
...
| `TenantService.HardDeleteAsync :511-549` | trusts the caller; deletes the owner folder before the transaction | when `reason == "demo_expired"`: **before `:549`** `if (!await Workspaces.IgnoreQueryFilters().AnyAsync(w => w.Id == workspaceId && w.DemoExpiresAt != null && w.DemoExpiresAt < DateTime.UtcNow)) return Result.Failure("Not an expired demo (converted or extended meanwhile).");` and **inside** `ExecuteInTransactionAsync`, first statement: `await _unitOfWork.ExecuteSqlRawAsync("SELECT id FROM workspaces WHERE id = {0} FOR UPDATE", workspaceId);` then the same `AnyAsync` again → on failure `throw new InvalidOperationException("demo converted during delete")` (rolls back; the hosted loop logs it and moves on). **Orchestrator 2026-09-23 (review finding #2):** the pre-check above closes the gap between the sweep's id query and this call, but not the (narrower) gap between the pre-check and the `FOR UPDATE` lock itself — for `demo_expired` specifically, the `tenant.hard_deleted` audit write moves **inside** `ExecuteInTransactionAsync`, after the locked re-check and the deletes' own `SaveChangesAsync` (so an abort at the locked re-check leaves no audit row at all, not merely one written-then-orphaned), and `DeleteOwnerFilesAsync` moves to **after** the transaction commits (so an abort never touches the filesystem either; a post-commit delete failure is reclaimed later by DB-16's orphan sweep). Every other `reason` keeps today's order (audit + files before the transaction) — the locked re-check only exists for `demo_expired` |
| `CommentService.cs:165-190` cap | `CurrentAdminAsync(owner)?.User.IsDemo/…Override` | `var demoWs = await _unitOfWork.Workspaces.IgnoreQueryFilters().Where(w => w.Id == owner && w.DemoExpiresAt != null && w.DemoConvertedAt == null).Select(w => new { w.DemoCommentCapOverride }).FirstOrDefaultAsync(); if (demoWs != null) { cap = demoWs.DemoCommentCapOverride ?? setting; … }` (same message; review finding #6 adds the `DemoConvertedAt == null` guard — a converted-but-unverified workspace, `Demo:ConvertRequiresVerification` mode, is not a demo for the cap even while `DemoExpiresAt` is still non-null for the re-verification grace) |
| `ExportImportService.cs:592-598` import cap | same shape | same replacement (+ the same `DemoConvertedAt == null` guard) |
...
7. `grep -c "FromMinutes(15)" API/Hosted/DemoCleanupService.cs` → 1; `grep -c "WarnExpiringAsync\|SweepThrottleRowsAsync" API/Hosted/DemoCleanupService.cs` → 1 each;
   **updated by the DB-17 review (findings #4/#10) — "every non-operator mint" replaces the earlier literal grep-count parity, which broke once the
   super-admin MFA-completion path stopped calling the guard with a permanently-empty set (finding #10; that path mints with no workspace at all and
   was never a real gap):** every non-operator token mint that HAS a workspace to check calls the guard before minting — `AuthService.LoginAsync`,
   `SwitchWorkspaceAsync`, the quick-access login path, the key-login path, the magic-link login path (all via `ExpiredDemoWorkspaceIdsAsync`), and
   `InviteService.AcceptJoinExistingWorkspaceAsync` (an inline `DemoExpiresAt < now` check on the invite's own workspace before doing any other work —
   review finding #4: an addressed invite into an already-expired demo must not mint a token either, since the sweep may delete that workspace within
   15 minutes regardless of what the invite just created); `grep -c "ExpiredDemoWorkspaceIdsAsync(Array.Empty<Guid>())" Application/Services/Implementation/AuthService.cs` → 0 (the no-op removed);
   `grep -c "DemoExpiresAt" Application/Services/Implementation/InviteService.cs` → ≥ 1;
   `grep -c "OnEmailVerifiedAsync" Application/Services/Implementation/EmailVerificationService.cs Application/Services/Implementation/AuthService.cs` → 1 each; `grep -c "NotifyDemoEmailVerifiedAsync(identity" Application/Services/Implementation/InviteService.cs` → 5 (review finding #7 — every addressed-invite `EmailVerifiedAt` flip in this service calls the shared best-effort wrapper, which itself calls `IDemoService.OnEmailVerifiedAsync` once); `grep -c "DEMO_CONVERT_REQUIRES_VERIFICATION" docker-compose.prod.yml .env.prod.example` → 1 each.
...
1. **Prod pre-checks (read-only, pasted into the PR):** §3.2 **Q1** (every listed row must be a `Demo Workspace`), **Q2** (expected 0; otherwise hand-delete or explain), **Q3** (expected 0), **Q4** (review finding #3 — every `blocks_tagging = true` row must correspond to a workspace Q1 does NOT list; a `false` row is a demo-own quick-access/invited member and is expected to appear next to a workspace Q1 DOES list), and `SELECT count(*) FROM app_settings WHERE key LIKE 'demo_email_%';` (the number of PII rows the first sweep will delete).
...
### 12.1 Post-commit code review (2026-09-23, applied on top of `cb2180a`)

A second review pass, after `cb2180a` (this doc's implementation) landed — Opus (MERGE WITH FIXES) + Gemini Pro (MERGE WITH FIXES) — found one HIGH and
several MEDIUM/LOW issues in the commit as written. All applied in a follow-up commit; adjudication:

| Finding | Claim | Verdict | Where it landed |
|---|---|---|---|
| **HIGH** | `DemoCleanupService.SweepOnceAsync` shares one `DbContext`/`IUnitOfWork` across every workspace in the sweep loop (regression from the pre-DB-17 job, which took a fresh scope per item); combined with `UnitOfWork.ExecuteInTransactionAsync`'s rollback path not clearing the change tracker, a failed item's queued deletes could replay into the next workspace's transaction | **Accepted** — restored a fresh `IServiceScopeFactory` scope PER ITEM (`IUnitOfWork`/`ITenantService` resolved inside the loop, not once outside it); `ExecuteInTransactionAsync` now clears the tracker in its rollback (`catch`) path too, belt-and-braces; the unused `demoService` param dropped from `SweepOnceAsync`'s signature | `API/Hosted/DemoCleanupService.cs`, `Infrastructure/Repository/UnitOfWork.cs`, `Tests/Db17HardDeleteAndCleanupTests.cs` |
| MEDIUM | `TenantService.HardDeleteAsync(reason: "demo_expired")` deletes the owner's files and writes the `tenant.hard_deleted` audit row BEFORE the locked re-check inside the transaction — a delete the locked re-check then aborts has already destroyed the files (and, before this review, already written the audit row) | **Accepted, `demo_expired` only** — `DeleteOwnerFilesAsync` moved to after the transaction commits (DB-16's orphan sweep reclaims a leftover if the post-commit delete itself fails); the audit write moved inside the transaction, after the locked re-check and the deletes' own `SaveChangesAsync`. Every other `reason` is unaffected (kept today's order — there is no locked re-check to race for those) | `Application/Services/Implementation/TenantService.cs`, §3.3 `HardDeleteAsync` row, `Tests/Db17HardDeleteAndCleanupTests.cs` |
| MEDIUM | The backfill's `NOT EXISTS` rejects *any* second live member — but a demo admin can mint their own quick-access link or addressed invite, so a demo with one of those (a real, demo-own member) would silently be skipped and never expire | **Accepted, orchestrator 2026-09-23: demo-own members don't block tagging** — narrowed to reject only a second live member whose identity's `owner_id` IS DISTINCT FROM `w.id` (a member genuinely at home elsewhere); added pre-check Q4 (§3.2, §9 step 1) | §3.2 (statement + Q4), migration `Up()`, rehearsal proof (see report) |
| MEDIUM | `InviteService`'s existing-workspace accept (`AcceptJoinExistingWorkspaceAsync`) issues a full token into an already-expired demo — it was never added to the `_tokenService.Issue(` guard sweep because it doesn't go through `AuthService` | **Accepted** — an inline `Workspaces.IgnoreQueryFilters().AnyAsync(w => w.Id == ownerId && w.DemoExpiresAt != null && w.DemoExpiresAt < now)` check before any other work, → `Failure(Demo.DemoExpired)`; acceptance criterion 7 (§7) rewritten around "every non-operator mint", since the super-admin MFA-completion no-op call it used to count on is now gone (next row) | §7 crit. 7, `Application/Services/Implementation/InviteService.cs` |
| MEDIUM (test debt) | `DemoCleanup_SkipsWorkspaceConvertedBetweenQueryAndDelete` modelled the race with a fake `ITenantService`, never exercising the real `HardDeleteAsync`; `CommentCap`/`ImportCap` had no test pinning the workspace-override read path; no operator-extend-then-self-extend test; the Guid `ExtendDemoAsync`/`SetDemoConfigAsync` operator paths had thin coverage | **Accepted** — rewritten with the REAL `TenantService` (conversion lands between the id query and the per-item re-check); added `CommentCap_UsesWorkspaceOverride_NotUser`, `ImportCap_UsesWorkspaceOverride`, `OperatorExtend_ThenSelfExtend_Fails`, and Guid-route operator tests | `Tests/Db17HardDeleteAndCleanupTests.cs`, `Tests/Db17DemoServiceTests.cs` |
| LOW | With `Demo:ConvertRequiresVerification` on, a converted-but-unverified workspace (`DemoExpiresAt` still non-null for the 72h grace) is not "still a demo" for extension or the comment cap | **Accepted** — `DemoService.ExtendAsync`/`TenantService.ExtendDemoAsync` refuse with `Demo.AlreadyUpgraded` when `DemoConvertedAt != null`; `CommentService`/`ExportImportService` cap queries add `&& w.DemoConvertedAt == null` | §3.3 rows (Extend, cap), the four services |
| LOW | `OnEmailVerifiedAsync` calls were unguarded (a throw there would fail the confirming action it rides along with); `InviteService`'s five addressed-invite `EmailVerifiedAt` sites never called it at all | **Accepted** — wrapped in try/catch (log `PublicId`) at both existing call sites (`AuthService`, `EmailVerificationService`); `EmailVerificationService`'s service-provider path additionally logs a Warning (not a silent skip — Gemini's ask) when `IDemoService` cannot be resolved from a REAL provider; `InviteService` gained a nullable `IDemoService`/`ILogger` seam and a shared best-effort `NotifyDemoEmailVerifiedAsync` wrapper called from all five sites | §7 crit. 7, `AuthService.cs`, `EmailVerificationService.cs`, `InviteService.cs` |
| LOW | `WarnExpiringAsync` had no per-workspace isolation (one workspace's failure stopped the whole warning pass); used the global TTL setting instead of a per-workspace override; always offered "Extend once" even when already used | **Accepted** — per-workspace try/catch around the whole loop body; `w.DemoTtlHoursOverride ?? globalTtlHours`; `BuildExpiryWarningHtml` takes a `canExtend` flag and omits the sentence when `DemoExtendedAt != null` | `Application/Services/Implementation/DemoService.cs` |
