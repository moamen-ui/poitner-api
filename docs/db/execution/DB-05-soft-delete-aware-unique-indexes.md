...
Review findings: S-4, S-5, M-2. Rules: R4 (small tables → plain ops), R9, R10, R11, R13.
**Class: index Expand/Contract in one migration** (each old unique index is replaced by a new one
that is at least as strict on live rows). One migration; no table data changes.

## 1. Goal

A soft-deleted user, role, environment, status label, extension origin, subscription or role
override no longer blocks re-creating the same value in the same workspace (today: unique
violation → raw 500). The `owner_id IS NULL` "global bucket" uniqueness that four hand-written
...
1. 7 × `DropIndex` (EF names: `IX_users_email_owner_id`, `IX_roles_name_owner_id`, `IX_app_environments_name_owner_id`, `IX_status_presentations_status_value_owner_id`, `IX_extension_sites_owner_id_origin`, `IX_subscriptions_owner_id`, `IX_role_tenant_overrides_role_id_owner_id` — confirm against the generated file; EF derives them from the snapshot).
2. 7 × `CreateIndex(… unique: true, filter: "deleted_at IS NULL")`, four of them with `.Annotation("Npgsql:NullsDistinct", false)`.
...
1. Edit the seven mapping lines per §3 (replace in place; keep surrounding lines).
2. `just migrate name="SoftDeleteAwareUniqueIndexes"`; open it; verify `Up()` = 7 `DropIndex` + 7 `CreateIndex` (4 with the NullsDistinct annotation) and nothing else; append the four `Sql` drops; mirror in `Down()`; add the marker. Any extra operation → stop and report (a stray `AlterColumn` here would mean a mapping typo).
3. `Tests/SoftDeleteUniqueIndexTests.cs` (§6). 4. `just fmt`, `just test`. 5. Rehearsal with the pre-checks, then `\di+ ux_*` shows the seven new indexes and `\di ix_*_global` shows none.
...
1. `grep -c 'HasFilter("deleted_at IS NULL")' Infrastructure/Mappings/*.cs` increased by exactly 7 versus `main`.
2. `grep -c "_global" Infrastructure/Migrations/*_SoftDeleteAwareUniqueIndexes.cs` → 8 (4 in `Up`, 4 in `Down`).
3. Snapshot contains the seven `ux_*_live` names and no `IX_users_email_owner_id` etc.
4. All four new tests pass; `just test` green.
5. Rehearsal: pre-checks return no rows; after update `SELECT indexname FROM pg_indexes WHERE indexname LIKE 'ix_%_global'` → 0 rows; inserting a second live user with an existing email in psql fails with `ux_users_email_owner_live`.

## 8. Rollback

`Down()` recreates the seven unfiltered indexes and the four raw ones. Rollback **fails** if, in
...
`git pull` → `stop api` → `backup-db.sh pre-db05` → run the seven pre-check queries on prod (all
empty) → `up -d --build api` → log grep → smoke: invite/create a user with an e-mail that belonged
to a deleted user in the same workspace (should succeed).

## 10. Out of scope

`projects.key` index (already correct), `api_keys`, `workspace_settings`, `invites.code`,
`quick_access_links.token_hash`, `device_logins`, `plans`, `app_settings.key` (not soft-deleted in
practice; leave), any service-layer uniqueness check, `clients/`, dashboard.
