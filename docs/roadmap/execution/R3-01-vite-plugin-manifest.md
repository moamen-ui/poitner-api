...
Plugin hooks: `enforce: 'pre'`; `transform(code, id)` for matching ids (uses `@babel/parser`? **No** — zero-dep rule: Decision: use Vite's bundled **esbuild** is not a JSX AST tool; use **`acorn`+`acorn-jsx`**? also a dep). **Decision:** the plugin may depend on `@babel/parser` + `@babel/traverse` + `@babel/generator` + `@babel/plugin-syntax-typescript`/`jsx` **as `peerDependencies` (optional)** — Vite React projects already have `@babel/core` via `@vitejs/plugin-react`. For `.vue` use `@vue/compiler-sfc` as an optional peer. The CLI's own zero-dep rule is preserved for `dist/cli.js`; the `vite` subpath lists these peers and fails with a clear message if missing. `transformIndexHtml` injects `<html data-build-sha="<sha>">` (attribute on the root `<html>` tag; created via the `html` hook string replace `/<html(\s|>)/`). `buildEnd`/`closeBundle` writes the manifest; in dev (`configureServer`) the manifest is rewritten on each file change (debounced 250 ms).
...
- Written atomically (`tmp` + rename). Regenerated by every dev/build run. `pointer doctor` and `pointer apply` regenerate it when missing/stale by running `npx vite build --mode pointer-manifest`? **Decision:** they do **not** run a build (too slow/side-effectful); they run the same transform **offline**: `pointer map --from-source` walks `include` globs with the same visitor and writes the manifest (no bundling). This shares `cli/src/vite/stamp.ts` between the plugin and the CLI.
...
**Decision — status model:** do **not** add a `Deployed` enum value. `CommentStatus` is used as an int by the widget (`StatusStr` map), dashboard filters, `pointer.sh`, skill.md and the apply-queue query (`Status == 2`); a new value is additive in EF but would silently fall through switch statements in three clients. Use **`Comment.DeployedAt: DateTime?`** + **`Comment.CommitSha: string?`** (nullable, additive). "Deployed" is derived (`Status == Applied && DeployedAt != null`).
...
- `POST /api/projects/{key}/builds` `[Authorize]` — `ReportBuildRequest { Sha: string, ContainsCommitShas?: string[] }` → `ReportBuildResponse { Sha, FirstSeen: bool, DeployedCommentIds: int[] }`.
  - Upserts `ProjectBuild`.
  - **Widget path** (`ContainsCommitShas` absent): marks `DeployedAt` for Applied comments of this project whose `CommitSha == Sha` exactly.
  - **CLI path (primary)**: the CLI computes, for every Applied-but-not-deployed comment (`GET /api/projects/{key}/comments?status=3` filtered client-side by `deployedAt == null`), `git merge-base --is-ancestor <commitSha> <sha>` and sends the passing ones in `ContainsCommitShas`; server marks them `DeployedAt = now, DeployedSha = Sha`.
  - Rate limit: reuse the `plans` policy shape → new `builds` policy 30/min per user.
- `UpdateCommentStatusRequest` gains `CommitSha?: string` (validator: `^[0-9a-f]{7,40}$`).
...
12. `Application/DTOs/Project/ReportBuildRequest.cs`, `ReportBuildResponse.cs`; `Application/Services/Interfaces/IProjectBuildService.cs`; `Application/Services/Implementation/ProjectBuildService.cs` — upsert + both marking paths (§F); `API/Controllers/ProjectBuildsController.cs` — `POST /api/projects/{key}/builds` `[Authorize]`, `[EnableRateLimiting("builds")]`, `[ProducesResponseType(typeof(ReportBuildResponse), 200)]`. `API/Extensions/RateLimitingExtensions.cs` — `builds` policy 30/min partitioned by user id claim (`sub`).
...
- **Unit (cli, vitest):** `hash.test.ts` (determinism, path normalisation Windows→POSIX, `default` export), `stamp.test.ts` (single root, Fragment multi-root, component-only root → no stamp, conditional/ternary/map roots, existing attribute preserved, `memo()` HOC, TSX generics, Vue multi-root), `manifest.test.ts` (atomic write, prev rotation, stale detection), `resolve.test.ts` (hash/path/legacy/stale).
- **Unit (API, xUnit):** `ProjectBuildServiceTests` (upsert idempotent; widget path exact-match marks; CLI path `ContainsCommitShas` marks; cross-tenant project key → NotFound; `Sha` validation), `CommentCommitShaTests` (PATCH persists `CommitSha` only when Applied; DTOs round-trip), `RateLimitingTests` extension for `builds`.
- **Integration (cli):** build the `e2e/fixture-app` Vite variant with the plugin → assert every rendered component root has `data-component-source` of 8 hex chars, `<html data-build-sha>` present, manifest entries == distinct components.
...
- [ ] Same repo cloned on two machines → byte-identical `entries` in `.pointer/manifest.json`.
