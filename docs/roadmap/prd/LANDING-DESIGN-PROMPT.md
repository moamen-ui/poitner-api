...
Pointer's apply flow is **cost-aware**, not just token-aware: the AI tool's premium/expensive model
...
rotatable) · Dashboards in Angular, React and Vue at parity · **Write feedback in your own language —
...
- **"Pointer in numbers"** — a public, thresholded stats band, live from `GET /api/public/stats`
...
- **Live data hooks to preserve:** pricing rendered from `GET https://api.pointer.moamen.work/api/plans`
  ...
  the widget: `<script src="https://api.pointer.moamen.work/pointer.js">` + `<pointer-feedback
