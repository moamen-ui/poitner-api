...
- R1-03-01: stack up, **plus R1-03's annotation task merged** — the guard is red against today's tree
  for three independent reasons, all fixed by that task:
  1. `CommentsController.Create` and `.List` carry **no `[ProducesResponseType]`**
     (`API/Controllers/CommentsController.cs:13-24`) → Swashbuckle emits a 200 with no `content`.
  2. `GET /api/branding` is annotated with the **wrapper**,
     `[ProducesResponseType(typeof(Result<BrandingResponse>), 200)]`
     (`API/Controllers/BrandingController.cs:35`) → step 3 sees an envelope-shaped component.
  3. `GET /api/admin/projects` returns `List<ProjectResponse>` → array schema (handled by the
     `items.$ref` fallback in step 2, no code change needed).
  The task is listed in `../execution/R1-03-dashboard-quickstart-key.md` (annotate the two comment
  actions with their inner types; de-wrap the branding annotation).
- R1-04 merged (`/api/meta` is in the endpoint list).
...
|---|---|---|---|---|---|---|---|
| R1-03-01 | swagger contract guard (Orval surface) | PR | api | — | 1. `GET http://localhost:8090/swagger/v1/swagger.json` (anonymous). 2. For each `(method, path)` in `DASHBOARD_CONTRACT_ENDPOINTS`: `['get','/api/me/api-key']`, `['post','/api/me/api-key/regenerate']`, `['get','/api/branding']`, `['get','/api/admin/projects']`, `['post','/api/admin/projects']`, `['post','/api/auth/login']`, `['post','/api/auth/login-with-key']`, `['get','/api/projects/{key}/comments']`, `['post','/api/projects/{key}/comments']`, `['get','/api/meta']` (R1-04, same release): assert `spec.paths[path][method]` exists, that `responses['200'].content['application/json']` exists (**a missing `content` means the action carries no `[ProducesResponseType]` at all — report that as the failure message, do not let the step throw on `undefined`**), and that the schema resolves to a component: **`schema.$ref ?? schema.items?.$ref`** matches `^#/components/schemas/` — the `items` form is required because `GET /api/admin/projects` is annotated `typeof(List<ProjectResponse>)` (`API/Controllers/Admin/ProjectsController.cs:21`), so its 200 schema is `type: array` with `items.$ref`. 3. For each resolved component: `components.schemas[<Name>]` does **not** define properties `isSuccess`/`data` (not the `Result` envelope). 4. **Decision:** the list lives in `e2e/scripts/lib/constants.mjs` as `DASHBOARD_CONTRACT_ENDPOINTS`; a PR that adds a dashboard-consumed endpoint adds its row here in the same PR (the R1-01 freeze rule, applied to routes). | 1 → 200 with `paths` + `components.schemas`; 2 → every endpoint present with a component-resolving 200 schema; 3 → no envelope-shaped components in the list. **This scenario only goes green once R1-03's annotation task lands** (see Preconditions) — against today's tree `post|get /api/projects/{key}/comments` have no annotation and `/api/branding` is wrapper-annotated. | report row; missing names listed in `detail` |
