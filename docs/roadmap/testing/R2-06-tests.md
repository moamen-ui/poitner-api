...
- Fixture: alpha fixture on **4181** (`serve.mjs alpha 4181`, `PORTS.alpha`) for the widget rows — 4173 is reserved for the `smoke` fixture that R2-05 needs (`run-e2e.sh:23`).
- **Widget reveal + environment filter:** widget rows import `preAuthWidget(page, token, user)` from `e2e/widget/lib/auth.ts` (it sets `pointer_visible`, so the toolbar renders; without it only `#pf-launcher` exists — `element.ts:161-163,649-655`), register `waitForResponse('**/capture-config')` **before** `page.goto`, and — because comment F is created with `environment: 1` while the alpha fixture mounts `environment="staging"` — switch first with `await widget.locator('#pf-env').selectOption('local')` (`fetchComments()` filters by `environmentInt`, `element.ts:618`).
...
|---|---|---|---|---|---|---|---|
| R2-06-01 ⛓ | `flag: secret in comment shows badge in widget` | PR | widget | QA | 1. QA `POST /api/projects/e2e-alpha/comments { body: 'leaked key ghp_<36 chars> please rotate', environment: 1, element: { selector: '.token', route: '/' } }` → id F. 2. Widget: `addInitScript`-free; `preAuthWidget(page, qaToken, qaUser)`; register `const cfg = page.waitForResponse('**/capture-config')`; `page.goto('http://localhost:4181/')`; `await cfg`; `await widget.locator('#pf-env').selectOption('local')` (F is env 1); `widget.locator('#pf-toggle').click()`. 3. Card: `widget.locator('.pf-card[data-id="<F>"] .pf-pill.pf-payload-flag')` (**Decision:** pill class `pf-payload-flag`, `--pf-warn` muted colour — this contract originally pinned `pf-flag`, but the widget has shipped `pf-payload-flag` since before the contract was written (web-component/src/templates.ts:198, styles/_pins.scss:40). Corrected here rather than renaming a class that is already in the built artifacts.). 4. QA `POST /api/comments/{F}/replies { body: '<script>alert(1)</script>' }`; refresh the list (`#pf-refresh`). 5. Reply pill on the same card. | 3 → pill visible with text `⚠ contains a secret/payload?` and `title` attribute containing `github_token`; 5 → reply carries the same pill shape with `title` containing `script_tag` (detector runs on replies — `AddReplyAsync` path) | report row; widget DOM dump |
| R2-06-02 ⛓ | `flag: absent from pointer get --json and pointer.sh get` | PR | cli | WA | 1. Temp repo: `.pointer/config.json` (project `e2e-alpha`), `credentials.env` with `keys.json.wsAdmin`; export `POINTER_SERVER=http://localhost:8090`, `POINTER_PROJECT=e2e-alpha` **and `POINTER_API_KEY=<keys.json.wsAdmin>`** — `pointer.sh:50` reads the key from the environment or `.pointer/credentials.env`, and `install.sh` writes that file **empty** (`install.sh:63-68`), so without this the legacy path cannot authenticate. 2. `spawnCli({ cwd: repo, args: ['get', String(F), '--json'] })`. 3. `installPointerSh(repo)`; run `bash .pointer/pointer.sh get <F>` (its token cache at `.pointer/.token_cache` is fresh). 4. Count `payloadFlag` occurrences (case-sensitive substring) in both outputs' stdout+stderr. | 2 → exit 0; the flag count is **0** and the parsed keys match `AiCommentView` (cross-ref R2-01-06 — the CLI prints the whitelist and never sends the header); 3 → exit 0, output **contains the comment body** (proves it actually fetched F) yet the flag count is **0**; 4 → both zero | report row; the two `grep -c payloadFlag` results (0 and 0) |
| R2-06-03 ⛓ | header gate both ways + zero-flag AI surfaces (summary, apply-queue, MCP) | PR | api | QA, WA, DEV | Raw `fetch` throughout (substring semantics): 1. `GET /api/comments/{F}` headers `{ Authorization: Bearer <qa>, 'X-Pointer-Client': 'widget' }` → body text contains `"hasPayloadFlag":true` and `github_token`. 2. Same URL **without** the header → `payloadFlag` substring count 0 (keys absent via `WhenWritingNull`, not `null`-valued). 3. `GET /api/projects/e2e-alpha/comments?view=summary` (QA token, with **and** without the header) → count 0 both times (fields do not exist on `CommentSummaryDto` — DTO-shape guarantee #1). 4. **Make F queue-visible first** — the apply-queue only ever returns `status=2` items (`Admin/ProjectsController.cs:99-108`), and F was created Open, so: WA `PATCH /api/comments/{F} { status: 2 }`, then `GET /api/admin/projects/e2e-alpha/apply-queue?status=2` (WA token, **admin-gated** — the endpoint is `[Authorize(Policy="Admin")]`) → the response **contains F's id** (otherwise the zero-count proves nothing) and `payloadFlag` count 0; items' embedded replies are `ApplyReplyDto` (`authorName, body, createdAt`) — assert a reply-bearing queued item also yields 0. 5. List DTO gate: `GET /api/projects/e2e-alpha/comments` with `X-Pointer-Client: widget` (QA) → the flagged item's JSON contains `"hasPayloadFlag":true` (`CommentListItemDto` honours the same gate). 6. MCP: `connectMcp({ cwd: mcpRepo })` (DEV key) → `callTool('pointer_get_comment', { id: F })` → serialized result `payloadFlag` count 0 (cross-ref R2-02-03). | 1 → true + pattern name present; 2/3/6 → **0** substrings each; 4 → F present in the queue **and** 0 substrings; 5 → true. The header is forgeable by design — the guarantee being asserted is that the *documented AI paths* (summary, apply-queue, CLI, MCP, legacy `pointer.sh`) never see it, stacked with the DTO/projection rules | report row; per-surface `grep -c payloadFlag` list (all 0, header case > 0) |
| R2-06-04 ⛓ | `flag: edit removes secret → flag cleared on reload` | PR | api + widget | QA | 1. QA `PUT /api/comments/{F}` `{ body: 'safe text now, token removed' }` → 200. 2. `GET /api/comments/{F}` with `X-Pointer-Client: widget` → raw text: no `hasPayloadFlag` key at all, no `github_token`. 3. Widget: `page.reload()` → re-boot; re-select `local` in `#pf-env` and re-open the list (`#pf-toggle`); `widget.locator('.pf-card[data-id="<F>"] .pf-pill.pf-payload-flag')` → `toHaveCount(0)`. **Note:** F is `status=2` after R2-06-03 step 4, so the card is still listed (the widget list is not status-filtered), but re-select the environment — a reload resets `environmentInt` to the attribute value. 4. Re-add via edit: `PUT` body `rotate again ghp_<new 36 chars>` → `GET` with header → `"hasPayloadFlag":true` again (edit recomputes, not create-only). | 1–3 → flag cleared end-to-end; 4 → flag returns on re-edit | report row; before/after raw JSON snippets |
...
## State coupling
```
R2-06-01 <- reset           # adds a flagged comment to `e2e-alpha` that later item counts depend on
R2-06-02 <- R2-06-01        # reads the comment R2-06-01 created
R2-06-03 <- R2-06-01        # PATCHes that comment to ReadyToApply for the apply-queue assertion
R2-06-04 <- R2-06-01        # edits that comment to clear the flag
```
