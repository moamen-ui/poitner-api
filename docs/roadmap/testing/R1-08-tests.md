...
## State coupling
```
R1-08-03 <- reset           # creates a global `Plan` row
R1-08-06 <- R1-08-01        # mutates invite rows directly; must follow every scenario that lists pending invites
R1-08-10 <- reset           # flips the global `emailEnabled` setting and owns the mail phase's tail
R1-08-12 <- reset           # mints a tenant through the direct path; the address then already owns a workspace
R1-08-16 <- R1-08-10        # the second of the two `emailEnabled` flips — they must run back-to-back
```
