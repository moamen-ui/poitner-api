...
- `docker compose exec -T db psql -U pointer -d pointer` is the SQL channel (compose env:
  ...
  `users.api_key`); the assertions are identical either way. **Follow-up (execution doc):** mirror this correction into `../execution/R1-06-api-key-hardening.md`'s acceptance criteria so the two SQL snippets agree — the test doc is right, the execution doc's PascalCase is the error.
...
|---|---|---|---|---|---|---|---|
| R1-06-01 | DB is hash-only + indexes + boot warning | nightly | api | — | After reset+seed: 1. `docker compose logs api 2>&1 \| grep -c 'set Auth:ApiKeyEncryptionKey for production'`. 2. `psql -tAc "SELECT count(*) FROM users WHERE api_key IS NOT NULL"`. 3. `psql -tAc "SELECT count(*) FROM api_keys"`. 4. With `K = keys.json.wsAdmin.apiKey` (node): `psql -tAc "SELECT prefix, hash, scopes, revoked_at FROM api_keys WHERE prefix = '${K.slice(0,12)}'"`; compute `sha256hex(K)` locally (`crypto.createHash('sha256')`). 5. Plaintext scan: `psql -tAc "SELECT count(*) FROM api_keys WHERE hash ~ 'ptr_' OR encrypted ~ 'ptr_[0-9a-f]{40}'"` — **`prefix` must not be in this predicate**: by design `Prefix` is the key's first 12 chars, i.e. always `ptr_########` (R1-06 §A, `ProfileService.cs:73`), so `prefix ~ 'ptr_'` matches every row and the assertion could never be 0 on a correct implementation. The point of the scan is that no **full** key survives anywhere. 6. `psql -tAc "SELECT indexdef FROM pg_indexes WHERE tablename = 'api_keys'"`. | 1 → ≥ 1 (derived-key boot warning, §B). 2 → `0` (backfill nulled the legacy column). 3 → ≥ 8 (one row per seeded persona key; super-admin per H-03 decision). 4 → one row: `hash === sha256hex(K)`, `scopes === 7` (Full), `revoked_at` NULL. 5 → `0`. 6 → one def contains `UNIQUE` + `(hash)`; one contains `UNIQUE` + `(user_id)` + `WHERE (revoked_at IS NULL)`. | report row; all six outputs in `detail` |
| R1-06-02 ⛓ | `legacy-key-still-logs-in-after-upgrade` | nightly (`upgrade` job; manual fallback `run-e2e.sh --upgrade`) | api | wsAdmin | Dedicated nightly job `upgrade` (extends the R1-07 workflow). **Decision: pin the legacy ref to the last commit the *candidate's* `seed.mjs` can still drive** — workflow env `LEGACY_REF` = the newest `main` commit that is (a) before R1-06's `AddApiKeysTable` migration and (b) after every seed-visible API change the candidate seed depends on (today: R1-05's `PATCH … {enforceAllowedOrigins:true}` for the beta fixture, and the `keys.json` minting loop). Step 4 runs the *candidate's* seed against the *legacy* image, so a ref that predates those endpoints makes seeding fail with a 400/404 that looks like an upgrade bug. If no such commit exists (the two windows do not overlap), the job runs a **reduced seed** instead — `node e2e/scripts/seed.mjs --minimal`, which creates only the tenant owner + one project + `keys.json`, skipping every fixture the legacy API does not know. Record which mode was used in the report row; the job fails with instructions if `LEGACY_REF` no longer builds. 1. `git fetch --depth 1 origin "$LEGACY_REF" && git worktree add ../legacy "$LEGACY_REF"`; `docker build -t pointer-api:legacy --target final ../legacy`; `docker build -t pointer-api:candidate --target final .` (the published stage — the dev target bind-mounts source, which would defeat the two-image test). 2. Write `e2e/state/upgrade/compose.upgrade.yml`: `services: { api: { image: pointer-api:${API_TAG}, pull_policy: never, build: !reset [], volumes: [], environment: ["Email__Provider=smtp","Email__Smtp__Host=mailpit","Email__Smtp__Port=1025"] } }` — the empty `volumes` **drops the `./:/src` bind mount** so the image's code is what runs. **`build: !reset []` and `pull_policy: never` are mandatory**: the base service declares `build: { context: ., target: dev }` (`docker-compose.yaml:11`), so without the reset `docker compose up -d api` would silently *build* a `dev`-target image and tag it `pointer-api:legacy` / `:candidate`, replacing the two pre-built images and defeating the whole test; `pull_policy: never` makes a missing tag fail loudly instead of hitting a registry. (`Email__Enabled` is deliberately absent — nothing reads it; e-mail is enabled by the seed's `PUT /api/admin/settings`, see H-tests preconditions.) **`API_TAG` must be exported for *every* compose invocation in this job, including `stop` and `down`** — an unset var resolves the image to `pointer-api:` and the command errors. 3. Initial wipe (the only `down -v` of the job): `API_TAG=legacy docker compose -f docker-compose.yaml -f e2e/state/upgrade/compose.upgrade.yml down -v --remove-orphans || true`, then `API_TAG=legacy … up -d`; poll `GET /swagger/v1/swagger.json` until 200 (same loop as `reset.sh:34`). 4. **Reset + seed on the legacy image:** `node e2e/scripts/seed.mjs` (its logins run against the legacy API); `rawLegacyKey = state/keys.json.wsAdmin.apiKey` — minted by the legacy image, i.e. stored the pre-upgrade way. 5. Swap images preserving everything: `docker compose -f docker-compose.yaml -f e2e/state/upgrade/compose.upgrade.yml stop api` (db + mailpit + `pgdata` volume untouched) → `API_TAG=candidate docker compose -f docker-compose.yaml -f e2e/state/upgrade/compose.upgrade.yml up -d api` — same compose project, **same volume, never `down -v` between the halves** (00-HARNESS §9). 6. Re-wait `/swagger/v1/swagger.json` (≤ 120 s — the candidate boots, migrates, and runs `ApiKeyBackfillService`). 7. `POST /api/auth/login-with-key {apiKey: rawLegacyKey}` → `data.token`. 8. With that token: `GET /api/me/api-key`. 9. `psql -tAc "SELECT count(*) FROM users WHERE api_key IS NOT NULL"` and `… FROM api_keys WHERE prefix = '<rawLegacyKey.slice(0,12)>' AND revoked_at IS NULL`. Steps 7–9 live in `e2e/scripts/upgrade-assert.mjs` (records into `state/report.md`). | 7 → 200, `data.status === 'ok'`, `data.user.email === 'e2e-owner@example.com'` (AC-3). 8 → 200, `data.apiKey === rawLegacyKey` (AC-4 — backfill encrypted the same raw key). 9 → `0` and `1`. | job log + `report.md` artifact with the row |
| R1-06-03 | `regenerated-key-old-one-rejected` | **nightly** — it shells out to `docker compose exec -T db psql` and **rewrites `state/keys.json`** mid-run; a PR-tier scenario with a cross-spec side effect is the wrong trade. (The pure-API half — old key rejected, new key works — could be PR-tiered later if the psql assertion moves into R1-06-01.) | api | pm | **Decision: use `pm`** — regenerating `wsAdmin`'s key would invalidate `keys.json` for every later spec. 1. `POST /api/auth/login` as pm → `GET /api/me/api-key` → `oldKey` (should equal `keys.json.pm.apiKey`). 2. `POST /api/me/api-key/regenerate` → `newKey`. 3. `POST /api/auth/login-with-key {apiKey: oldKey}`. 4. `POST /api/auth/login-with-key {apiKey: newKey}` → token; then `GET /api/me/api-key` with it. 5. `psql -tAc "SELECT count(*) FROM api_keys WHERE prefix = '<newKey.slice(0,12)>' AND revoked_at IS NULL"` and the same for `oldKey`'s prefix. 6. Rewrite `state/keys.json → pm.apiKey = newKey` (later phases keep working). | 2 → 200; `newKey` matches `/^ptr_[0-9a-f]{40}$/`, ≠ `oldKey`; `data.prefix === newKey.slice(0,12)`; `data.lastUsedAt` present after step 4 (additive fields, R1-06 task 8). 3 → **400**, envelope `isSuccess === false` (old key revoked; status-based assert, AC-5's `InvalidApiKey`). 4 → `status === 'ok'`; reveal returns `newKey`. 5 → `1` (new) and `0` for the old prefix's non-revoked count — exactly one active row. | report row |
...
## State coupling
```
R1-06-02 <- reset           # the two-image upgrade job owns its own compose project and volume lifecycle
R1-06-04 <- R1-04-04        # shares the three-restart budget; runs as step ② of the restart group
```
