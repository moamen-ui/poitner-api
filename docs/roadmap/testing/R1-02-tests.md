...
- AC-1 (fresh Vite app → init → widget renders → comment posted, ≤ 5 min) → **R1-02-01** · AC-2
  ...
  once after two comments) → **R1-02-05** · `/check` page (Design §J) → **R1-02-06** · the detected app-URL confirmation (Design §H2 / step 4b) → **R1-02-07**.
...
- **Known partial coverage of AC-1.** AC-1 says "*interactive*, no AI tool, ≤ 5 min", but R1-02-01
  drives `--yes --json` (a TTY prompt loop is not scriptable in CI). The prompt sequence, its exact
  texts, the 3-attempt key retry and the "✔ Signed in as …" line are proven only by
  `cli/test/init-yes.test.ts` against the stub server. What R1-02-01 does prove end-to-end is the
  non-interactive equivalent plus the real widget/comment outcome.
...
| R1-02-07 | `init-detects-local-url` | PR | cli | developer | In `e2e/cli/init.spec.mjs`, four sub-cases in one temp Vite-shaped fixture (no `npm install` — only the config files matter, `detectAppUrl` never runs a dev server): **(a) default** — fixture with a bare `vite.config.ts`; `spawnCli(initArgs({ yes: true, json: true }))` → parse stdout. **(b) custom port** — rewrite `vite.config.ts` to `export default { server: { port: 4000 } }`; re-run `init` in a fresh temp dir. **(c) explicit override** — same fixture, add `--app-url https://app.acme.test`. **(d) opt out** — same fixture, add `--no-app-url`, then `spawnCli(['doctor','--json'])`. After (a)-(c): `GET /api/admin/projects` as wsAdmin → the created project, then `GET /api/admin/projects/{id}/app-urls` to read what was stored for the chosen environment (R1-09 shape; if R1-09 has not landed, read `project.appUrl` instead and say so in the report). | (a) `json.appUrl === 'http://localhost:5173'`, `json.appUrlSource` names `vite.config.ts` or the default; stored URL matches. (b) `json.appUrl === 'http://localhost:4000'` — the config's port beats the default. (c) `json.appUrl === 'https://app.acme.test'` — the flag beats detection. (d) `json.appUrl === null`, no URL stored, and `doctor --json` has a `⚠` check mentioning the missing app URL, exit 0. No case ever yields `0.0.0.0` or `127.0.0.1`. | report row with all four detected values and their sources |
...
- R1-02-07 does **not** start a dev server and does **not** `npm install` — detection is pure file
  reading (Design §H2), so the scenario is seconds, not minutes. If it ever needs a real server it has
  been written wrong.

- Fresh-app scenarios run in CI only on the `cli/**` path filter (npm scaffolding is slow); the
  vite/next stacks are nightly regardless, so the PR tier stays ≤ 15 min even on a cold runner where
  `reset` alone pays for image pulls plus a full `dotnet watch` first build. Pinned **minor**
  generator versions; one whole-stack retry only on the 300 s budget; every attempt's seconds go to
  the CI log (**not** into `detail` — H-02's determinism diff forbids per-run values there).
...
## State coupling
```
R1-02-04 <- reset           # creates project `my-app`; a second run in the same stack conflicts (409)
R1-02-05 <- reset           # asserts `first_comment` fires exactly once — the first is unobservable twice
```
