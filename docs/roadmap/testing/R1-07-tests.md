...
| R1-07-03 | runner flags | manual | cli | — | 1. `bash e2e/run-e2e.sh --list --nightly` 2. `bash e2e/run-e2e.sh --list --pr` 3. `bash e2e/run-e2e.sh --bogus` 4. `bash e2e/run-e2e.sh --only R1-05-03` 5. bare `bash e2e/run-e2e.sh` | 1,2 → phase lists printed, nothing executed, nightly ⊃ pr, `--429` last in the nightly list; 3 → exit 2 naming `--bogus`; 4 → only that scenario runs (grep the report for other ids → none); 5 → the pre-change behaviour (reset → seed → probe → widget) | the two printed lists pasted in the report |
| R1-07-04 | evidence artifact from a zero-AI run | manual | cli | — | 1. `rm -f e2e/state/report.md` 2. `bash e2e/run-e2e.sh --ci --pr` (**no** `--with-ai`) 3. inspect `e2e/state/report.md` and `state/junit.xml` 4. re-run step 2 and diff the two reports after stripping the `ms` and `detail` columns | 3 → file non-empty, has the header (UTC ts, git sha, flags, `/api/meta` stack line, mailpit count), a `## Phases` row per phase of the tier incl. `SKIP` rows with reasons, and a `## Scenarios` row per executed id; `junit.xml` parses; 4 → identical (`H-02`) | both reports attached |
...
## State coupling
```
R1-07-01 <- reset           # dispatches two sequential full workflow runs; not re-runnable as a unit mid-run
```
