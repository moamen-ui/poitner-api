...
## State coupling
```
R1-04-03 <- R1-04-02        # reuses the `tempRepo` git identity R1-04-02 configured
R1-04-04 <- reset           # consumes one of the three permitted `api` restarts (the `Cli__MinVersion` window)
R1-04-05 <- reset           # burns the per-IP `meta` bucket; a retry inside the window re-429s
R1-04-06 <- R1-04-04        # publishes into, and asserts against, the `Cli__MinVersion` window R1-04-04 opens
```
