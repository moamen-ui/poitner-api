...
| Served URLs | `/widget.js`, `/widget.css` (current, 2026-09-17+ — the bundled file names, matching the widget's own naming), `/pointer.js`, `/pointer.css` (pre-rename names, served byte-identical forever), `/embed.js`, `/install.sh`, `/skill.md`, `/skills/apply.md`, `/skills/translate.md`, `/skills/advanced.md` (current, 2026-09-18+ — the `pointer-feedback` skill split into a small entry file plus these three sub-files; served with the same `<POINTER_SERVER>`/`<POINTER_PRODUCT>`/`<POINTER_SKILL_VERSION>` substitution as `/skill.md`), `/pointer-init.md`, `/pointer.sh`, `/vendor/snapdom.js` | API | keep as permanent aliases |
