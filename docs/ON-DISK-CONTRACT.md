...
| `config.json` keys | `server`, `project`, `aiTool`, `skillsDir` (optional), `cliVersion`, `delivery`, `delegation` (optional, 2026-09-19+ — `auto`\|`off`; absent means `auto`. Lets the apply skill hand mechanical edits to a cheaper worker model when the AI tool supports sub-agents; `off` makes it apply everything itself. Never written by `init` — a team sets it by hand; the CLI only reads it to print it in the `apply` prompt header), `projects` (optional — a multi-project repo's `{ key: { path, htmlPath, delivery } }` map, replacing `project`/`htmlPath` for that repo). `environment` (`local`\|`staging`\|`production`) and `environments` are **legacy — no longer written by `init`** since environments and their activation moved to the dashboard; both names stay frozen here (still read for backward compatibility on a config written by an older CLI) and the `<pointer-feedback>` `environment` attribute below is unaffected — it remains a supported, opt-in pin (`init --environment <list>`) | CLI | additive only |
...
| Served URLs | `/widget.js`, `/widget.css` (current, 2026-09-17+ — the bundled file names, matching the widget's own naming; the pre-rename `/pointer.js`/`/pointer.css` aliases were dropped 2026-09-19 — no consumer predates the rename, so there was nothing to keep working), `/embed.js`, `/install.sh`, `/skill.md`, `/skills/apply.md`, `/skills/translate.md`, `/skills/advanced.md` (current, 2026-09-18+ — the `pointer-feedback` skill split into a small entry file plus these three sub-files; served with the same `<POINTER_SERVER>`/`<POINTER_PRODUCT>`/`<POINTER_SKILL_VERSION>` substitution as `/skill.md`), `/pointer-init.md`, `/pointer.sh`, `/vendor/snapdom.js` | API | keep |
| API alias | `/api/v1/*` (rewrites to `/api/*`; R5-68) | API middleware | keep; future `/api/v2` if ever needed |
| Skill directories | `.claude/skills/pointer-init/`, `.claude/skills/pointer-feedback/`, `.agents/skills/pointer-init/`, `.agents/skills/pointer-feedback/` (current, 2026-09-16+ — the Agent Skills standard layout, used by `other`/`antigravity` and symlinked into from the three tools above) — **gitignored** (see the `.gitignore` row above); every clone/machine gets its own copy via `init`/`update`/`install.sh`. Since 2026-09-18, a folder-capable install (`claude-code`/`other`/`antigravity`, or any `--skills-dir` override) additionally writes `apply.md`, `translate.md`, `advanced.md` as siblings of `SKILL.md` **inside these same frozen folders** (no new directory names); a flat-file tool (`cursor`, `windsurf`) has no folder for siblings, so its `pointer-feedback.md` rules file concatenates all four sections into itself instead. `.agents/pointer-init/`, `.agents/pointer-feedback/` — **legacy location (pre-2026-09-16), removed on the next `init`/`update`** | `install.sh`, CLI | keep |
| Browser storage | `localStorage` `pointer_token`, `pointer_user` (`element.ts:413`), `pointer_env_<project>`, `pointer_toolbar_pos`, `pointer_widget_theme` (widget's own light/dark override — see the `data-fbk-theme` footnote), `pointer_widget_language` (widget's own en/ar override, same reasoning — never written to `User.language`); `sessionStorage` `pointer_visible`, `pointer_page_session_id` (`pagecontext.ts:179`), `pointer_lang_reported` (2026-09-18+ — guards the once-per-tab-session `widget_language` usage event posted to the existing `POST /api/events`) | widget | keep (or accept one re-login) |
