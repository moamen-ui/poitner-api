...
| `delegation` | optional, **never ask for it.** `auto` (or absent) lets the apply skill hand mechanical edits to a cheaper worker model when the AI tool supports sub-agents — the orchestrating model still plans and reviews; `off` makes it apply everything itself. A team sets it by hand later if they want it off; `init` does not write it |
