...
case "${1:-list}" in
  ...
  queue)
    ...
    if echo "$ADMIN_QUEUE" | jq -e '.isSuccess' >/dev/null 2>&1; then
      ...
      echo "$ADMIN_QUEUE" | jq '(.data.pages // {}) as $pages | [.data.items[] | {id, status: (if .status==1 then "Open" elif .status==2 then "ReadyToApply" elif .status==3 then "Applied" elif .status==4 then "Archived" else "Other" end), environment: (if .environment==1 then "Local" elif .environment==2 then "Staging" else "Prod" end), body, prompts: [.pickedActions[]?.prompt], aiRules: [.aiRules[]? | "[\(.title)] \(.prompt)"], route: ($pages[.element.pageRef].route // .element.pageRef), file: .element.sourcePath}]'
    else
