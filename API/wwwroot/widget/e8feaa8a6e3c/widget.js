...
`).slice(1);
  ...
  var COMMIT_STYLE_CONTROL_ENABLED = !1, PIN_CLUSTER_RADIUS = 24, PIN_HALF_WIDTH = 16, PIN_HEIGHT = 30, PIN_TOOLTIP_HEIGHT_ESTIMATE = 150, PIN_TOOLTIP_WIDTH = 220, PIN_TOOLTIP_EDGE_MARGIN = 8, _PointerFeedback = class _PointerFeedback extends HTMLElement {
    ...
    async _boot() {
      ...
      this.token && this._reportBuildSha(), this.token && this._reportWidgetLanguage(), inviteFailed && this.toast("This invite link is invalid or expired — ask for a new one.", "error");
    }
    /**
    ...
    saveAuth(token, user) {
      this.token = token, this.user = user, this.shortcut = parseShortcut(user == null ? void 0 : user.addCommentShortcut), localStorage.setItem("pointer_token", token), localStorage.setItem("pointer_user", JSON.stringify(user)), this.startNotificationPolling();
    }
    clearAuth() {
