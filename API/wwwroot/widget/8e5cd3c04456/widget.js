...
(() => {
  ...
  var CSS_INTEGRITY = true ? "sha384-QYqtfnoKxMe1dZUCmLykm3Kv0tcZ01NjHemSjrihCFYpY/3DsXYxXV8nvcNVt7xZ" : "";
...
@media (prefers-reduced-motion: reduce){
  ...
  var _PointerFeedback = class _PointerFeedback extends HTMLElement {
    ...
    // --- Theme ----------------------------------------------------------------
    // An explicit account choice (this.user.theme, set from the user menu below OR the dashboard's
    // own theme toggle — same field) always wins; otherwise the host page's own rendered theme,
    // falling back to the OS preference when the page doesn't paint one explicitly. See theme.ts.
    resolveTheme() {
      var _a2;
      const stored = (_a2 = this.user) == null ? void 0 : _a2.theme;
    ...
    // Persists an explicit theme choice to the account (PATCH /api/me/preferences) so it follows the
    // user across browsers/machines and into the dashboard — mirrors saveShortcutPreference's shape.
    async saveThemePreference(mode) {
      try {
        ...
        if (!r.ok) return false;
        if (this.user) {
          this.user = { ...this.user, theme: mode };
          localStorage.setItem("pointer_user", JSON.stringify(this.user));
        }
        this.applyTheme();
        return true;
    ...
    toggleUserMenu() {
      ...
      const wireThemeBtn = (id, mode) => {
        host.querySelector(id).addEventListener("click", async (e) => {
          ...
          const ok = await this.saveThemePreference(mode);
          if (ok) reopenUserMenu();
          else this.toast("Failed to save theme — try again", "error");
        });
      };
    ...
    async hydrateIdentity() {
      ...
      try {
        ...
        this.user = {
          ...
          theme: this.user && this.user.theme || me.theme,
