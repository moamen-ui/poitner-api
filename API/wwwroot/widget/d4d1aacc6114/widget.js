...
(() => {
  ...
  var POSITIONS = ["top-start", "top-end", "bottom-start", "bottom-end"], SHOT_MAX_WIDTH = 1280, SHOT_HIGHLIGHT = "#2563eb", _a, SCRIPT_SRC = ((_a = document.currentScript) == null ? void 0 : _a.src) || "", CSS_INTEGRITY = "sha384-Q3Fi1LRBq0YXvMiCgQj8Tz28JDQrTV3O+z7DPLlYGtULLyCmgFUM/z5sixS5de1Z";
...
@media (prefers-reduced-motion: reduce){
  ...
  ${displayName ? `
  <button type="button" class="fbk-toolbar-btn fbk-toolbar-btn--icon" id="fbk-updates" data-fbk-act="updates" aria-expanded="false" aria-haspopup="dialog" data-toggle="tooltip" data-placement="top" title="${t("toolbar.recentActivityUpdates")}" aria-label="${t("toolbar.updates")}${unreadSuffix(unreadNotifyCount)}"><span class="fbk-toolbar-btn__icon">${ICON.bell}</span><span class="fbk-toolbar-dot${unreadNotifyCount > 0 ? "" : " fbk-hidden"}" id="fbk-notify-count" data-fbk-unread aria-hidden="true"></span></button>
  <span class="fbk-toolbar__divider" aria-hidden="true"></span>
...
`).slice(1);
  ...
  // src/auth-ui.ts
  async function apiSwitchWorkspace(server, workspaceId, selectionToken) {
  ...
  function renderWorkspacePicker(host, workspaces, selectionToken) {
    let body = host.root.querySelector("#fbk-auth-body");
    if (!body) return;
    body.innerHTML = TPL.workspacePicker(workspaces);
    let errEl = body.querySelector("#fbk-login-error"), backBtn = body.querySelector("#fbk-show-login");
    backBtn && backBtn.addEventListener("click", () => renderLoginView(host));
    let items = body.querySelectorAll(".fbk-workspace-item");
    items.forEach((item) => {
      item.addEventListener("click", async () => {
        let workspaceId = item.getAttribute("data-workspace-id");
        if (workspaceId) {
          items.forEach((btn) => {
            btn.disabled = !0;
          }), errEl.textContent = t("auth.switchingWorkspace");
          try {
            let r = await apiSwitchWorkspace(host.server, workspaceId, selectionToken), envelope = await r.json(), data = envelope.data || null;
            if (r.ok && data && data.token) {
              afterAuthOk(host, data.token, data.user);
              return;
            }
            errEl.textContent = envelope.message || t("auth.invalidCredentials"), items.forEach((btn) => {
              btn.disabled = !1;
            });
          } catch {
            errEl.textContent = t("auth.networkError"), items.forEach((btn) => {
              btn.disabled = !1;
            });
          }
        }
  ...
  function renderLoginView(host, opts = {}) {
    ...
    let emailEl = body.querySelector("#fbk-email"), passEl = body.querySelector("#fbk-password"), errEl = body.querySelector("#fbk-login-error"), submitBtn = body.querySelector("#fbk-login-submit"), doLogin = async () => {
      ...
      try {
        let envelope = await (await host.apiLogin(email, password, host.project)).json(), data = envelope.data || null, status = data && data.status;
        ...
        if (status === "no-workspace") {
          errEl.textContent = envelope.message || t("auth.noWorkspace"), restore();
          return;
        }
  ...
  var COMMIT_STYLE_CONTROL_ENABLED = !1, PIN_CLUSTER_RADIUS = 24, PIN_HALF_WIDTH = 16, PIN_HEIGHT = 30, PIN_TOOLTIP_HEIGHT_ESTIMATE = 150, PIN_TOOLTIP_WIDTH = 220, PIN_TOOLTIP_EDGE_MARGIN = 8, _PointerFeedback = class _PointerFeedback extends HTMLElement {
    ...
    async _checkWidgetActive() {
      ...
      try {
        ...
        if (!res.ok) return !1;
        let body = await res.json(), data = (_a2 = body == null ? void 0 : body.data) != null ? _a2 : body;
        return this._paused = (data == null ? void 0 : data.paused) === !0, (data == null ? void 0 : data.active) === !0;
      } catch {
        return !1;
      }
    }
    ...
    _handlePaused() {
      this._paused || (this._paused = !0, this.renderChrome(), this.renderSidebar(), this.renderPins(), this.toast(t("paused.notice")));
    }
    _stylesReady() {
    ...
    // --- API ----------------------------------------------------------------
    async apiLogin(email, password, projectKey = this.project) {
    ...
    api(path, opts = {}) {
      ...
      return pfFetch(`${this.server}${path}`, { ...opts, headers }).then((r) => {
        ...
        return r.status === 423 && this._handlePaused(), r;
      });
    }
    /**
    ...
    renderChrome() {
      ...
      if (this.root.classList.toggle("fbk-paused", this._paused), this._collapsed) {
      ...
      resetBtn && resetBtn.addEventListener("click", () => this.resetToolbarPos()), this.restoreToolbarPos(), this.enableToolbarDrag(), this.updateMenuSide(), this.renderPausedNotice();
    }
    ...
    renderPausedNotice() {
      let el = this.root && this.root.querySelector("#fbk-paused-notice");
      el && (el.classList.toggle("fbk-hidden", !this._paused), this._paused && (el.textContent = t("paused.notice")));
    }
    ...
    async uploadToServer(blob) {
      ...
      try {
        ...
        if (r.status === 423)
          return this._handlePaused(), null;
        if (!r.ok) throw new Error("HTTP " + r.status);
        let envelope = await r.json();
    ...
    async createComment(data) {
      ...
      if (data.attachShot && data.shotPromise) {
        ...
        if (blob) {
          ...
          url ? element.screenshotUrl = url : this._paused || this.toast(t("toast.screenshotUploadFailed"), "error");
        }
      }
    ...
    async addReply(id, text) {
      try {
        let r = await this.api(`/api/comments/${id}/replies`, {
          method: "POST",
