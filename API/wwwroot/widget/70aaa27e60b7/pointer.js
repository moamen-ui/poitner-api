...
(() => {
  ...
  var TPL = {
    ...
    card: (c, i, isQuickAccess) => {
      ...
      const verifyGroup = c.status === "applied" && !c.verifiedAt && c._canVerify ? `<span class="pf-verify-group">
      ...
      const verifyBox = c.status === "applied" && !c.verifiedAt && c._canVerify ? `<div class="pf-verify-box pf-hidden" id="pf-verify-box-${c.id}">
    ...
    async fetchCaptureConfig() {
      ...
      try {
        ...
        if (!this.environmentExplicit && typeof resolved === "number" && ENV_NAME[resolved]) {
          ...
          const envSel = this.root && this.root.querySelector("#pf-env");
