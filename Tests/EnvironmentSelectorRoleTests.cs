...
public class EnvironmentSelectorRoleTests
{
    ...
    public async Task Unconfigured_QuickAccessCaller_DoesNotSeeSelector()
    {
        ...
        var clientSvc = new ProjectService(
            new UnitOfWork(BuildContext(client, dbName)),
            client,
            new PassThroughEntitlements(),
            TestProjectServiceDeps.Settings(),
            TestProjectServiceDeps.Configuration(),
    ...
    public async Task Configured_CallerRoleInList_SeesSelector()
    {
        ...
        await svc.UpdateAsync(
            ...
            {
                EnvironmentSelectorRoleIds = new List<int> { 5, 9 },
            }
        );

        var roleFive = new FakeCurrentUser
        {
            Id = Guid.NewGuid(),
            TenantId = tenant,
            RoleId = 5,
        };
        var roleFiveSvc = new ProjectService(
            new UnitOfWork(BuildContext(roleFive, dbName)),
            roleFive,
            new PassThroughEntitlements(),
            TestProjectServiceDeps.Settings(),
            TestProjectServiceDeps.Configuration(),
    ...
    public async Task Configured_CallerRoleNotInList_DoesNotSeeSelector()
    {
        ...
        var roleTwo = new FakeCurrentUser
        {
            Id = Guid.NewGuid(),
            TenantId = tenant,
            RoleId = 2,
        };
        var roleTwoSvc = new ProjectService(
            new UnitOfWork(BuildContext(roleTwo, dbName)),
            roleTwo,
            new PassThroughEntitlements(),
            TestProjectServiceDeps.Settings(),
            TestProjectServiceDeps.Configuration(),
    ...
    public async Task UpdateAsync_EmptyList_ClearsBackToDefault()
    {
        ...
        await svc.UpdateAsync(
            created.Id,
            new UpdateProjectRequest { EnvironmentSelectorRoleIds = new List<int> { 5 } }
        );
        var afterSet = await svc.UpdateAsync(
            created.Id,
            new UpdateProjectRequest { EnvironmentSelectorRoleIds = new List<int>() }
        );
