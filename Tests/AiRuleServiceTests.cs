...
public class AiRuleServiceTests
{
    ...
    public async Task Admin_CanCreateTenantAndProjectRules()
    {
        ...
        // 1. Tenant admin rule
        var tenantRes = await svc.CreateAsync(
            new CreateAiRuleRequest
            {
        ...
        var projRes = await svc.CreateAsync(
            new CreateAiRuleRequest
            {
                ProjectId = 1,
    ...
    public async Task Developer_CanCreatePersonalRule_AppliedOnlyToTheirComments()
    {
        ...
        var dev1RuleRes = await dev1Svc.CreateAsync(
            new CreateAiRuleRequest
            {
                ProjectId = 10,
                Title = "Dev 1 Rule",
    ...
    public async Task Developer_CannotEditOrDeleteAdminRule()
    {
        ...
        var updateRes = await devSvc.UpdateAsync(
            created.Id,
            new UpdateAiRuleRequest { Title = "Hacked" }
        );
        ...
        using (var db = BuildContext(admin, dbName))
        {
            db.Projects.Add(
                new Project
                {
                    ...
                    Name = "P1",
                    OwnerId = tenant,
                    AiToolsUsed = "[\"claude-code\", \"cursor\"]",
                }
            );
            db.Projects.Add(
                new Project
    ...
    // DB-13 (F2): a plain (non-impersonating) super admin still gets the platform-wide COUNTS
    // (metadata) but never rule text (content) — DetailedRules/RecentRules are empty. An
    // impersonating operator, pinned to the session's workspace, gets that workspace's rule text.
    [Fact]
    public async Task SuperAdmin_PlainOperator_GetsCountsButNoRuleText_Impersonating_GetsTargetDetails()
    {
        var dbName = Guid.NewGuid().ToString();
        ...
        // Counts/tenant summaries are metadata — unaffected.
        ...
        // Rule text is content — a plain operator gets none, even with includeDetails: true.
        Assert.Null(insights.DetailedRules);
        Assert.Empty(insights.RecentRules);

        // Impersonating tenant1: pinned to that workspace's rule text.
        ...
        var impSvc = new AiRuleService(
            new UnitOfWork(BuildContext(impersonating, dbName)),
            impersonating
        );
        var impInsights = (
            await impSvc.GetInsightsAsync(tenantId: tenant2, includeDetails: true)
        ).Data!;
        // effectiveTenantId is pinned to the session's workspace (tenant1), NOT the tenantId param.
        Assert.Equal(1, impInsights.TotalRulesCount);
        Assert.NotNull(impInsights.DetailedRules);
        Assert.Single(impInsights.DetailedRules);
        Assert.Equal("T1 Rule", impInsights.DetailedRules[0].Title);
    }

    [Fact]
    public async Task ListAllRules_PlainOperator_Forbidden_Impersonating_PinnedToTenant()
    {
        var dbName = Guid.NewGuid().ToString();
        var tenant1 = Guid.NewGuid();
        var tenant2 = Guid.NewGuid();
        ...
        // Plain operator: refused outright — rule text is content, not something "all workspaces" ever shows.
        var svc = new AiRuleService(new UnitOfWork(BuildContext(superAdmin, dbName)), superAdmin);
        var forbidden = await svc.ListAllRulesAsync();
        Assert.True(forbidden.IsForbidden);

        // Impersonating tenant1: sees only tenant1's rules, pinned regardless of the tenantId param.
        var impersonating = new FakeCurrentUser
        {
            Id = superAdmin.Id,
            IsAdmin = true,
        ...
        var t1Rules = (await impSvc.ListAllRulesAsync(tenantId: tenant2)).Data!;
        Assert.Single(t1Rules);
        Assert.Equal("T1 P Rule", t1Rules[0].Title);
    }
