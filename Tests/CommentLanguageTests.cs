...
public class CommentLanguageTests
{
    ...
    private static CreateCommentRequest Req(string body, string? language) =>
        new()
        {
            Body = body,
    ...
    public async Task Language_RoundTrips_ToCommentResponse()
    {
        ...
        var result = await h.CommentService.CreateAsync(
            "proj",
            Req("مرحبا بكم في هذا الموقع", "ar"),
            h.AuthorId
        );
    ...
    public async Task Language_RoundTrips_ToSummaryItem()
    {
        ...
        var summary = await h.CommentService.ListSummaryAsync(
            "proj",
            new CommentFilter { View = "summary" },
            h.AuthorId
        );
