...
public class InviteJoinUrlBaseTests
{
    ...
    private static async Task<string> CreateInviteUrlAsync(string dbName, DictSettings settings)
    {
        ...
        var result = await service.CreateAsync(
            new CreateInviteRequest { RoleId = roleId, Email = "invitee@acme.test" }
        );
    ...
    public async Task Uses_BrandUrlApp_When_AppBaseUrl_Is_Unset()
    {
        ...
        var settings = new DictSettings().Set(
            ISettingsService.BrandUrlApp,
            "https://app.acme.test"
        );

        var url = await CreateInviteUrlAsync(
            nameof(Uses_BrandUrlApp_When_AppBaseUrl_Is_Unset),
            settings
        );
    ...
    {
        var url = await CreateInviteUrlAsync(
            nameof(Falls_Back_To_Compiled_Default_When_Nothing_Is_Configured),
            new DictSettings()
        );
    ...
    public async Task Whitespace_Only_Override_Falls_Through_To_Branding()
    {
        ...
        var url = await CreateInviteUrlAsync(
            nameof(Whitespace_Only_Override_Falls_Through_To_Branding),
            settings
        );
