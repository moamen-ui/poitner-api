...
public class TokenServiceTests
{
    [Fact] public void Issue_includes_sub_role_email_and_admin_flag()
    {
        ...
        var token = svc.Issue(new User { Id = 7, Email = "a@b.c", DisplayName = "A", RoleId = role.Id, Role = role }, null);
    ...
    public void Issue_includes_is_super_admin_true_when_role_is_super_admin()
    {
        ...
        var token = svc.Issue(new User { Id = 1, Email = "sa@test.com", DisplayName = "SA", RoleId = role.Id, Role = role }, null);
        var jwt = new JwtSecurityTokenHandler().ReadJwtToken(token);
    ...
    public void Issue_includes_tenant_claim_when_membership_is_present_and_omits_it_when_null()
    {
        ...
        var membership = new WorkspaceMembership { Id = 9, OwnerId = tenantId, RoleId = role.Id, Role = role, SecurityStamp = Guid.NewGuid() };

        // Membership present: should have tenant claim
        var tokenWithTenant = svc.Issue(new User { Id = 5, Email = "dev@tenant.com", DisplayName = "Dev", RoleId = role.Id, Role = role, OwnerId = tenantId }, membership);
        ...
        var tokenNoTenant = svc.Issue(new User { Id = 6, Email = "global@test.com", DisplayName = "Global", RoleId = role.Id, Role = role, OwnerId = null }, null);
    ...
    public void Issue_WithMembership_EmitsTenantAndMstamp()
    {
        var opts = Options.Create(new JwtOptions { SigningKey = new string('k', 40), Issuer = "pointer-api", LifetimeHours = 12 });
        var svc = new JwtTokenService(opts);
        var identityRole = new Role { Id = 1, Name = "Workspace Admin", GrantsAdmin = true };
        var membershipRole = new Role { Id = 2, Name = "Developer", GrantsAdmin = false };
        var tenantId = Guid.NewGuid();
        ...
        var user = new User { Id = 5, Email = "dev@tenant.com", DisplayName = "Dev", RoleId = identityRole.Id, Role = identityRole };
    ...
    public void Issue_includes_security_stamp_claim()
    {
        ...
        var token = svc.Issue(new User { Id = 7, Email = "a@b.c", DisplayName = "A", RoleId = role.Id, Role = role, SecurityStamp = stamp }, null);
        var jwt = new JwtSecurityTokenHandler().ReadJwtToken(token);
