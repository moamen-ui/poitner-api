...
public class CommentService : ICommentService
{
    ...
    )
    {
        ...
        if (!projectResult.IsSuccess)
            return projectResult.IsConflict
                ? Result<CommentResponse>.Conflict(
        ...
        var environment =
            request.Environment == EnvironmentTag.Unknown
        ...
        if (environment != EnvironmentTag.Unknown)
        {
            ...
            if (!gated.IsSuccess)
                return gated.IsConflict
                    ? Result<CommentResponse>.Conflict(
                        gated.Message ?? MessageKeys.Project.Disabled
                    )
                    : Result<CommentResponse>.NotFound(
                        gated.Message ?? MessageKeys.Project.NotFound
                    );
        }
        ...
        if (
            !await _projectService.IsOriginAllowedAsync(
                projectResult.Data,
                origin,
                environment,
                _currentUser.IsQuickAccess
            )
        )
        ...
        var projectInfo = await _unitOfWork
            .Repository<Project>()
            .Query()
        ...
        var fieldDefs = await _commentFields.GetDefinitionsForOwnerAsync(
            projectOwnerId,
            enabledOnly: true
        );
        ...
        {
            // DB-17 §3.3: the WORKSPACE is the demo authority now (never the demo-flag/override
            // fields on the identity). DB-17 review finding #6: a converted workspace is not a
            // demo for the cap even while Demo:ConvertRequiresVerification keeps DemoExpiresAt
            // non-null for the re-verification grace.
            ...
            if (demoWs != null)
            {
                ...
                if (count >= cap)
                    return Result<CommentResponse>.Failure(
                        $"Demo limit reached: a demo workspace allows at most {cap} comments."
                    );
            }
        }
        ...
        if (projectOwnerId is Guid planOwner)
        {
            ...
            var monthCount = await _unitOfWork
                .Repository<Comment>()
                .Query()
                .IgnoreQueryFilters()
                .CountAsync(c =>
                    c.OwnerId == planOwner && c.DeletedAt == null && c.CreatedAt >= monthStart
                );
            var check = await _entitlements.CheckCountAsync(
                planOwner,
                EntitlementCatalog.MaxCommentsPerMonth,
                monthCount
            );
            if (!check.IsSuccess)
                return Result<CommentResponse>.LimitReached(
        ...
        comment.Element.Snapshot = SnapshotSanitizer.Sanitize(
            request.Element.Snapshot,
            projectInfo.CaptureTextContent
        );
        ...
        if (
            request.IsBugReport
            && projectInfo.PageContextCaptureEnabled
            && request.PageContext is { } capture
            && !string.IsNullOrWhiteSpace(capture.SessionId)
        )
        {
            var route = NormalizeRoute(request.Element.Route);
            var pageContext = await _unitOfWork
                .Repository<PageContextSnapshot>()
                .Query()
                .Where(s =>
                    s.ProjectId == projectResult.Data
                    && s.Route == route
        ...
        if (request.PredefinedActionIds is { Count: > 0 } actionIds)
        {
            foreach (var actionId in actionIds.Distinct())
            {
                ...
                var action = await _predefinedActions.ResolveInScopeAsync(
                    actionId,
                    projectResult.Data,
                    projectOwnerId,
                    authorId
                ...
                if (action == null)
                    return Result<CommentResponse>.Failure(
                        MessageKeys.Comment.InvalidPredefinedAction
                    );

                comment.PickedActions.Add(
                    new CommentPickedAction { Text = action.Text, Prompt = action.Prompt }
                );
            }
        }
        ...
        try
        {
            ...
            {
                Type = UsageEventTypes.FirstComment,
                Source = "api",
                ProjectId = projectResult.Data,
                OwnerId = projectOwnerId,
        ...
        return Result<CommentResponse>.Success(
            MapToResponse(comment, names, fieldDefs: fieldDefs),
            MessageKeys.Comment.Created
        );
    }
    ...
    public async Task<Result<PagedData<CommentListItemDto>>> ListAsync(
        string projectKey,
        CommentFilter filter,
        Guid callerId
    )
    {
        ...
        if (!projectResult.IsSuccess)
            return projectResult.IsConflict
                ? Result<PagedData<CommentListItemDto>>.Conflict(
                    projectResult.Message ?? MessageKeys.Project.Disabled
                )
                : Result<PagedData<CommentListItemDto>>.NotFound(
                    projectResult.Message ?? MessageKeys.Project.NotFound
                );
        ...
        IQueryable<Comment> query = BuildCommentQuery(projectId, filter, callerId)
            .Include(c => c.Replies);
        ...
        var hiddenPrivateCount = await query.CountAsync(c => c.IsPrivate && c.AuthorId != callerId);
        ...
        return Result<PagedData<CommentListItemDto>>.Success(
            new PagedData<CommentListItemDto>(
                items.Select(c => MapToListItem(c, names, fieldDefs)).ToList(),
                pagination,
                hiddenPrivateCount,
                pageContexts
            )
    ...
    public async Task<Result<PagedData<CommentSummaryDto>>> ListSummaryAsync(
        string projectKey,
        CommentFilter filter,
        Guid callerId
    )
    {
        ...
        if (!projectResult.IsSuccess)
            return projectResult.IsConflict
                ? Result<PagedData<CommentSummaryDto>>.Conflict(
                    projectResult.Message ?? MessageKeys.Project.Disabled
                )
                : Result<PagedData<CommentSummaryDto>>.NotFound(
                    projectResult.Message ?? MessageKeys.Project.NotFound
                );
        ...
        return Result<PagedData<CommentSummaryDto>>.Success(
            new PagedData<CommentSummaryDto>(items, pagination, hiddenPrivateCount)
        );
    }

    public async Task<Result<PagedData<CommentApplyItemDto>>> ListApplyQueueAsync(
        string projectKey,
        CommentFilter filter
    )
    {
        ...
        if (!projectResult.IsSuccess)
            return projectResult.IsConflict
                ? Result<PagedData<CommentApplyItemDto>>.Conflict(
                    projectResult.Message ?? MessageKeys.Project.Disabled
                )
                : Result<PagedData<CommentApplyItemDto>>.NotFound(
                    projectResult.Message ?? MessageKeys.Project.NotFound
                );
        ...
        var aiRules = await _unitOfWork
            ...
            .Where(r =>
                ...
                && (
                    ...
                    || (
                        r.UserId != null
                        && authorIds.Contains(r.UserId.Value)
        ...
        var personalRulesByAuthor = aiRules
            .Where(r => r.UserId != null)
            .GroupBy(r => r.UserId!.Value)
            .ToDictionary(
                g => g.Key,
                g =>
                    g.OrderBy(r => r.SortOrder)
                        .ThenBy(r => r.CreatedAt)
                        .Select(r => new AiRuleApplyDto
                        {
                            Title = r.Title,
        ...
        return Result<PagedData<CommentApplyItemDto>>.Success(
            new PagedData<CommentApplyItemDto>(
                items
                    .Select(c =>
                    {
                        var commentRules = adminRules
                            .Concat(
                                personalRulesByAuthor.GetValueOrDefault(c.AuthorId)
                                    ?? Enumerable.Empty<AiRuleApplyDto>()
                            )
                            .ToList();
                        return MapToApplyItem(
                            c,
                            names,
                            pageRefByCommentId.GetValueOrDefault(c.Id),
                            commentRules,
                            fieldDefs
                        );
                    })
    ...
    public async Task<Result<CommentResponse>> GetByIdAsync(int id, Guid callerId)
    {
        ...
        var effectiveRules = await _unitOfWork
            .Repository<AiRule>()
            .Query()
            .AsNoTracking()
            .Where(r =>
                ...
                && (
                    ...
                    || (
                        r.UserId == comment.AuthorId
                        && (r.ProjectId == null || r.ProjectId == comment.ProjectId)
                    )
                )
            )
            .OrderBy(r => r.UserId == null ? (r.ProjectId == null ? 0 : 1) : 2) // Strict priority: Workspace (0) > Project (1) > Personal (2)
        ...
        return Result<CommentResponse>.Success(
            MapToResponse(comment, names, effectiveRules, fieldDefs)
        );
    }

    public async Task<Result<CommentResponse>> UpdateStatusAsync(
        int id,
    ...
    )
    {
        ...
        if (_currentUser.IsQuickAccess)
            return Result<CommentResponse>.Forbidden(
                MessageKeys.Comment.QuickAccessCannotChangeStatus
            );
        ...
        if (request.Status == CommentStatus.Applied)
        {
            try
            {
                ...
                {
                    Type = UsageEventTypes.FirstApply,
                    Source = "api",
                    ProjectId = comment.ProjectId,
                    OwnerId = comment.OwnerId,
                    CreatedAt = DateTime.UtcNow,
        ...
        var updatedFieldDefs = await _commentFields.GetDefinitionsForOwnerAsync(
            comment.OwnerId,
            enabledOnly: false
        );
        return Result<CommentResponse>.Success(
            MapToResponse(comment, names, fieldDefs: updatedFieldDefs),
            message
        );
    }

    public async Task<Result<CommentResponse>> VerifyAsync(
        int id,
        VerifyCommentRequest request,
        Guid actorId
    )
    {
        var comment = await _unitOfWork
        ...
        if (!isAuthor && !isAdmin)
            return Result<CommentResponse>.Forbidden(
                "You do not have permission to verify this comment."
            );
        ...
        if (request.Ok)
        {
            ...
            var replyText = !string.IsNullOrWhiteSpace(request.Note)
                ? request.Note.Trim()
                : "Verified ✓";
            var reply = new Reply
            {
        ...
        else
        {
            ...
            if (comment.AppliedBy.HasValue)
            {
                ...
                if (excerpt.Length > 80)
                    excerpt = excerpt[..80];
                var payload = new NotificationPayloadDto { ReplyExcerpt = excerpt };
                var notification = new Notification
                {
                    OwnerId = comment.OwnerId,
        ...
        var verifyFieldDefs = await _commentFields.GetDefinitionsForOwnerAsync(
            comment.OwnerId,
            enabledOnly: false
        );
        return Result<CommentResponse>.Success(
            MapToResponse(comment, names, fieldDefs: verifyFieldDefs),
            message
        );
    }

    public async Task<Result<CommentResponse>> EditAsync(
        int id,
    ...
    )
    {
        ...
        return Result<CommentResponse>.Success(
            MapToResponse(comment, editNames, fieldDefs: editFieldDefs),
            "Comment updated."
        );
    }
    ...
    public async Task<Result<CommentResponse>> UpdateFieldsAsync(
        int id,
        UpdateCommentFieldsRequest request,
        Guid actorId
    )
    {
        var comment = await _unitOfWork
        ...
        if (!_currentUser.IsAdmin && comment.AuthorId != actorId)
            return Result<CommentResponse>.Forbidden(
                "You do not have permission to edit this comment's fields."
            );
    ...
    public async Task<Result<CommentResponse>> SetVisibilityAsync(
        int id,
        Guid callerId,
        bool isPrivate
    )
    {
        ...
        if (comment.AuthorId != callerId)
            return Result<CommentResponse>.Failure(
                "You can only change the visibility of your own comments."
            );
        ...
        var names = await ResolveNamesAsync(AuthorIds(comment));
        var visibilityFieldDefs = await _commentFields.GetDefinitionsForOwnerAsync(
            comment.OwnerId,
            enabledOnly: false
        );
        return Result<CommentResponse>.Success(
            MapToResponse(comment, names, fieldDefs: visibilityFieldDefs)
        );
    }

    public async Task<Result<ReplyResponse>> AddReplyAsync(
        int commentId,
    ...
    public async Task<Result<ReplyResponse>> EditReplyAsync(
        int replyId,
        UpdateReplyRequest request,
        Guid editorId
    )
    {
        var reply = await _unitOfWork
            .Repository<Reply>()
            .Query()
    ...
    private static ElementCapture MapToEntity(ElementCaptureDto dto) =>
        new()
        {
    ...
    private ElementCaptureDto MapElementToDto(ElementCapture entity) =>
        new()
        {
    ...
    private static ReplyResponse MapReplyToResponse(
        Reply reply,
    ...
    private CommentListItemDto MapToListItem(
        Comment comment,
        IReadOnlyDictionary<Guid, string> names,
        IReadOnlyList<CommentFieldDefinition>? fieldDefs = null
    ) =>
        new()
        {
            Id = comment.Id,
            ...
            Replies = comment
                .Replies.Select(r => MapReplyToResponse(r, names, ShowsPayloadFlags))
                .ToList(),
            ...
            CustomFields = _commentFields.Resolve(
                fieldDefs ?? NoFieldDefinitions,
    ...
    private CommentResponse MapToResponse(
        Comment comment,
        IReadOnlyDictionary<Guid, string> names,
        List<AiRuleApplyDto>? rules = null,
        IReadOnlyList<CommentFieldDefinition>? fieldDefs = null
    ) =>
        new()
        {
    ...
    private ApplyElementDto MapToApplyElement(Comment comment, string? pageRef) =>
        new()
        {
            PageRef = pageRef,
        ...
        foreach (var comment in items)
        {
            ...
            if (!pageRefByKey.TryGetValue(pageKey, out var pageRef))
            {
                ...
                pages[pageRef] = new ApplyPageDto
                {
                    ...
                    Viewport =
                        element.ViewportWidth.HasValue && element.ViewportHeight.HasValue
    ...
    private CommentApplyItemDto MapToApplyItem(
        Comment comment,
        IReadOnlyDictionary<Guid, string> names,
        string? pageRef,
        List<AiRuleApplyDto>? rules = null,
        ...
        new()
        {
            ...
            Replies = comment
                .Replies.Select(r => new ApplyReplyDto
                {
            ...
            PickedActions = comment
                .PickedActions.Select(a => new PickedActionDto { Text = a.Text, Prompt = a.Prompt })
                .ToList(),
            AiRules = rules ?? new List<AiRuleApplyDto>(),
            IsBugReport = comment.IsBugReport,
            PageContextId = comment.PageContextSnapshotId,
    ...
    private static void MergePageContext(
        PageContextSnapshot snapshot,
        PageContextCaptureDto capture
    )
    {
        foreach (var c in capture.ConsoleEntries)
        {
            snapshot.ConsoleEntries.Add(
                new ConsoleLogEntry
                {
                    Level = c.Level,
                    Message = c.Message,
        ...
        if (snapshot.ConsoleEntries.Count > MaxPageContextEntries)
            snapshot.ConsoleEntries.RemoveRange(
                0,
                snapshot.ConsoleEntries.Count - MaxPageContextEntries
            );

        foreach (var n in capture.NetworkEntries)
        {
            snapshot.NetworkEntries.Add(
                new NetworkFailureEntry
                {
                    Method = n.Method,
                    Url = n.Url,
        ...
        if (snapshot.NetworkEntries.Count > MaxPageContextEntries)
            snapshot.NetworkEntries.RemoveRange(
                0,
                snapshot.NetworkEntries.Count - MaxPageContextEntries
            );
    }
    ...
    private async Task<IReadOnlyDictionary<int, PageContextDto>?> LoadPageContextsAsync(
        IEnumerable<int?> ids
    )
    {
        ...
        var snapshots = await _unitOfWork
            .Repository<PageContextSnapshot>()
            .Query()
            .AsNoTracking()
    ...
    private static PageContextDto? MapPageContextToDto(PageContextSnapshot? snapshot)
    {
        ...
        return new PageContextDto
        {
            ...
            ConsoleEntries = snapshot
                .ConsoleEntries.Select(e => new ConsoleEntryDto
                {
                    Level = e.Level,
                    Message = e.Message,
                    Stack = e.Stack,
                ...
                .ToList(),
            NetworkEntries = snapshot
                .NetworkEntries.Select(e => new NetworkEntryDto
                {
                    Method = e.Method,
                    Url = e.Url,
