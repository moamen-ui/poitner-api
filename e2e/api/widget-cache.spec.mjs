...
//
//   /widget.js              → no-cache               (the floating latest build)
//   /widget.js?v=stable     → public, max-age=3600   (opt into an hour of staleness)
//   /widget.js?v=<hash>     → immutable, one year    (pinned: the bytes can never change)
//
...
test('R3-03-06 — cache-header matrix (API hop)', async () => {
  ...
  const css = await head(`/widget.css?v=${H}`);
  ...
  if (H0) {
    const older = await head(`/widget.js?v=${H0}`);
  ...
  const unknown = await head('/widget.js?v=000000000000');
  ...
  for (const v of ['..%2F..%2Fetc%2Fpasswd', '%00', 'a'.repeat(200)]) {
    const bad = await head(`/widget.js?v=${v}`);
  ...
  const empty = await head('/widget.js?v=');
...
test('R3-03-06b — cache-header matrix (Caddy hop)', async () => {
  ...
  const bare = await through('/widget.js');
  expect(bare.status).toBe(200);
  ...
  const unknown = await through('/widget.js?v=000000000000');
  expect(unknown.status).toBe(404);
  ...
  const empty = await through('/widget.js?v=');
